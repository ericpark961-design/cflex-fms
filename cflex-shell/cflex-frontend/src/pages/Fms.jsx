...
function Admin({ user }) {
  ...
  const tabs = [
    ...
    { id: 'sso', label: tx('M365 SSO', 'M365 SSO') },
  ];
  return (
    <div style={{ padding: 24, maxWidth: 1100 }}>
      ...
      {tab === 'sso' && <M365SsoAdmin user={user} />}
    </div>
  );
}
...
// ─── Admin → M365 SSO subtab ──────────────────────────────────────
// Customer admins self-serve Microsoft 365 tenant connection,
// group→role mapping, and login policy.
const ROLE_OPTIONS = ['fms_admin', 'customer_admin', 'customer_viewer', 'engineer', 'noc_operator'];

function M365SsoAdmin({ user }) {
  const [cfg, setCfg] = useState(null);
  ...
  const save = async () => {
    ...
    setBusy(false);
    if (r?.ok) {
      setMsg({ ok: true, text: tx('저장 완료', 'Saved') });
      setForm(f => ({ ...f, client_secret: '' }));
      reload();
    } else {
      setMsg({ ok: false, text: r?.error || tx('저장 실패', 'Save failed') });
    }
  };
  ...
  const test = async () => {
    ...
    setBusy(false);
    if (r?.ok) setMsg({ ok: true, text: tx(`연결 성공 — issuer: ${r.issuer}`, `Connected — issuer: ${r.issuer}`) });
    else setMsg({ ok: false, text: r?.error || tx('연결 실패', 'Test failed') });
  };

  const disconnect = async () => {
    if (!confirm(tx('Microsoft 365 연결을 해제할까요? 매핑도 함께 삭제됩니다.',
                    'Disconnect Microsoft 365? Role mappings will also be removed.'))) return;
  ...
  const loadGroups = async () => {
    ...
    else setMsg({ ok: false, text: r?.error || tx('그룹 조회 실패', 'Group lookup failed') });
  };

  const addMapping = async () => {
    if (!newMap.ad_group_id) { setMsg({ ok: false, text: tx('그룹을 선택하세요', 'Pick a group') }); return; }
    ...
    else setMsg({ ok: false, text: r?.error || tx('추가 실패', 'Add failed') });
  };

  const delMapping = async (id) => {
    if (!confirm(tx('매핑을 삭제할까요?', 'Delete mapping?'))) return;
  ...
  return (
    <>
      <Section title={tx('Microsoft 365 연결', 'Microsoft 365 connection')}>
        <div style={{ background: C.card, border: `1px solid ${C.hairline}`, padding: 18 }}>
          <div style={{ marginBottom: 14, padding: '10px 14px', fontSize: 13,
                        background: connected ? C.okSoft : '#fff8e1',
                        border: `1px solid ${connected ? C.ok : C.warn}` }}>
            <strong style={{ color: connected ? C.ok : '#8a6d3b' }}>
              {connected ? tx('✓ 연결됨', '✓ Connected') : tx('○ 미연결', '○ Not connected')}
            </strong>
            {connected && cfg?.connected_at && (
              <span style={{ color: C.inkMuted, marginLeft: 10, fontSize: 12 }}>
                · {fmtAbs(cfg.connected_at)}
              </span>
            )}
          </div>

          <details style={{ marginBottom: 14, background: C.surface || '#f4f4f4', padding: 12 }}>
            <summary style={{ cursor: 'pointer', fontWeight: 600, fontSize: 12, color: C.inkMuted, textTransform: 'uppercase', letterSpacing: '0.4px' }}>
              {tx('Azure AD에서 App을 등록하는 방법', 'How to register an Azure AD app')}
            </summary>
            <ol style={{ fontSize: 12, color: C.inkMuted, lineHeight: 1.7, margin: '10px 0 0 18px' }}>
              ...
              <li>등록 후 <strong>Overview</strong> 페이지에서 <strong>Application (client) ID</strong>와 <strong>Directory (tenant) ID</strong> 복사 → 아래 입력</li>
              <li><strong>Certificates &amp; secrets</strong> → <strong>New client secret</strong> → 값 복사 (한 번만 보임) → 아래 입력</li>
              <li><strong>API permissions</strong> → Add permission → Microsoft Graph → Delegated: <code>User.Read</code> · Application: <code>GroupMember.Read.All</code> · <strong>Grant admin consent</strong></li>
              <li><strong>Token configuration</strong> → Add optional claim → ID → <code>groups</code> (그룹 매핑용)</li>
            </ol>
          </details>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field label={tx('Directory (Tenant) ID', 'Directory (Tenant) ID')}>
              <input style={fieldStyle} value={form.azure_tenant_id}
                     onChange={e => setForm({ ...form, azure_tenant_id: e.target.value })}
                     placeholder="00000000-0000-0000-0000-000000000000" />
            </Field>
            <Field label={tx('Application (Client) ID', 'Application (Client) ID')}>
              <input style={fieldStyle} value={form.client_id}
                     onChange={e => setForm({ ...form, client_id: e.target.value })}
                     placeholder="00000000-0000-0000-0000-000000000000" />
            </Field>
            <Field label={connected ? tx('Client Secret (변경 시에만 입력)', 'Client Secret (only when changing)')
                                     : tx('Client Secret', 'Client Secret')}>
              <input style={fieldStyle} type="password" value={form.client_secret}
                     onChange={e => setForm({ ...form, client_secret: e.target.value })}
                     placeholder={connected ? tx('비워두면 기존 값 유지', 'Leave empty to keep current') : ''} />
            </Field>
            <Field label={tx('허용 도메인 (쉼표 구분)', 'Allowed domains (comma-separated)')}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, fontSize: 13, cursor: 'pointer' }}>
            ...
            <span>{tx('일반 비밀번호 로그인 차단 (M365만 허용)',
                       'Disable password login (M365 only)')}</span>
          </label>

          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button onClick={save} disabled={busy || !form.azure_tenant_id || !form.client_id || (!connected && !form.client_secret)}
                    style={{ height: 36, padding: '0 18px', background: C.primary, color: '#fff',
                             border: 'none', cursor: busy ? 'wait' : 'pointer', fontWeight: 600 }}>
              {connected ? tx('업데이트', 'Update') : tx('연결 저장', 'Save & Connect')}
            </button>
            {connected && (
              <>
                <button onClick={test} disabled={busy} style={btnGhostMini}>{tx('연결 테스트', 'Test connection')}</button>
                <button onClick={disconnect} style={btnDangerMini}>{tx('연결 해제', 'Disconnect')}</button>
              </>
            )}
          </div>
      ...
      {connected && (
        <Section title={tx('Azure AD 그룹 → 역할 매핑', 'Azure AD group → role mapping')}>
          <div style={{ background: C.card, border: `1px solid ${C.hairline}`, padding: 18 }}>
            <div style={{ fontSize: 12, color: C.inkMuted, marginBottom: 12 }}>
              {tx('Azure AD 그룹 멤버는 로그인 시 매핑된 C-Flex 역할이 자동 부여됩니다.',
                  'When AD group members sign in, they receive the mapped C-Flex role automatically.')}
            </div>

            {/* Group picker — Graph API live lookup */}
            <div style={{ display: 'grid', gridTemplateColumns: '2fr auto', gap: 10, alignItems: 'flex-end', marginBottom: 12 }}>
              <Field label={tx('그룹 검색 (이름 시작)', 'Search groups (name prefix)')}>
                <input style={fieldStyle} value={groupSearch}
                       onChange={e => setGroupSearch(e.target.value)}
                       placeholder={tx('예: HAEA-FMS', 'e.g. HAEA-FMS')} />
              </Field>
              <button onClick={loadGroups} disabled={groupBusy} style={btnGhostMini}>
                {groupBusy ? tx('조회 중…', 'Loading…') : tx('그룹 가져오기', 'Fetch groups')}
              </button>
            </div>

            {groups.length > 0 && (
              <div style={{ marginBottom: 14, maxHeight: 220, overflow: 'auto',
                            border: `1px solid ${C.hairline}` }}>
                {groups.map(g => (
                  <div key={g.id} style={{ padding: '8px 12px', fontSize: 12, cursor: 'pointer',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 10, alignItems: 'flex-end', marginBottom: 14 }}>
              <Field label={tx('선택된 그룹', 'Selected group')}>
                <input style={fieldStyle} value={newMap.ad_group_name || newMap.ad_group_id}
                       ...
                       placeholder={tx('위에서 클릭하거나 그룹 ID 직접 입력', 'Click above or enter group ID')} />
              </Field>
              <Field label={tx('할당 역할', 'Assigned role')}>
              ...
              <button onClick={addMapping}
                      style={{ height: 36, padding: '0 18px', background: C.primary, color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                {tx('매핑 추가', 'Add mapping')}
              </button>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${C.hairline}`, paddingTop: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.inkMuted, textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 8 }}>
                {tx('등록된 매핑', 'Active mappings')} · {mappings.length}
              </div>
              {mappings.length === 0 ? (
                <div style={{ padding: 16, fontSize: 12, color: C.inkSubtle, textAlign: 'center', background: '#f4f4f4' }}>
                  {tx('매핑이 없습니다. 매핑이 없으면 신규 사용자는 customer_viewer 역할로 생성됩니다.',
                ...
                <div key={m.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 10,
                  ...
                  <button onClick={() => delMapping(m.id)} style={btnDangerMini}>{tx('삭제', 'Delete')}</button>
                </div>
              ))}
            </div>
          </div>
